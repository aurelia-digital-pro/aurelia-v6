...
// ── CONVERSATION MEMORY (table: core_memory) ─────────────────────────
...
export async function getTokenAwareHistory(sessionId, charBudget = TOKEN_BUDGET) {
  ...
  const { data, error } = await supabase.from('core_memory')
    .select('role, content').eq('session_id', sid)
    .order('created_at', { ascending: false }).limit(MEMORY_WINDOW);
...
export async function pruneHistory(sessionId, keep = 20) {
  const sid = sessionId || 'default';
  const { data } = await supabase.from('core_memory').select('id')
    .eq('session_id', sid).order('created_at', { ascending: false }).limit(200);
...
// ── EXECUTION MEMORY (table: execution_memory) ───────────────────────

export async function getExecutionMemory(sessionId) {
  const sid = sessionId || 'default';
  const { data, error } = await supabase.from('execution_memory')
    .select('type, key, value, created_at').eq('session_id', sid)
    .order('created_at', { ascending: false }).limit(EXEC_WINDOW);
...
export async function getRecentDecisions(sessionId) {
  const sid = sessionId || 'default';
  const { data, error } = await supabase.from('execution_memory')
    .select('value, created_at').eq('session_id', sid).eq('type', 'LOG_DECISION')
...
export async function getRecentTasks(sessionId) {
  const sid = sessionId || 'default';
  const { data, error } = await supabase.from('execution_memory')
    .select('value, created_at').eq('session_id', sid).eq('type', 'ADD_TASK')
    .order('created_at', { ascending: false }).limit(5);
  if (error) return [];
  return (data || []).reverse();
}
