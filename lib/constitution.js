...
// ── STATIC EXPORT (required by chat.js import) ──────────────────────
...
export async function getConstitution() {
  const { data, error } = await supabase
    .from('constitution').select('key, value');
...
export async function getActivePhase() {
  const { data, error } = await supabase
    .from('phase_tracker').select('phase_name, status')
...
export async function getNextPhase() {
  const { data, error } = await supabase
    .from('phase_tracker').select('phase_name')
